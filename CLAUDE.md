...
| R16 (OPEN; **pre-existing, not a recent regression**) | UI lags for ~18-20 s after window appears on 300+ tab sessions — p95 input latency 1.0-1.2 s, sustained-fast never reached. Window is "technically responsive" (WM_NULL gets through between WM_TIMER ticks) but each user click/keystroke queues behind the current pump tick, so the user feels "not responding". Stock NPP is FULLY frozen for ~20 s then snaps to snappy; we're ~1-2 s faster to snappy but laggy throughout. **Verified to be present in the very first R14 commit (92ec06f)** too: p95=1041 ms, sustained=18109 ms. The lag was always there — the old smoke metric just hid it by probing "WM_NULL answered within 500 ms" (yes — WM_TIMER is lowest priority, WM_NULL skips ahead between ticks). The fork's real win over stock is "no Not Responding badge" and "active tab loads in 1-2 s sync", not "instantly snappy". Suspected hot paths: (a) `addBufferAt` with explicit `sessionIndex` calls `TCM_INSERTITEM` in the middle of the tab strip → O(remaining) per insert → O(N²) over the pump; (b) `applyLazyContent` runs on main thread on every NPPM_INTERNAL_LAZYLOADWORKERDONE — `SCI_APPENDTEXT` for each file's bytes is non-trivial. Fix candidates: batch tab UI inserts, defer `applyLazyContent` until tab is viewed, throttle worker callbacks so pump can drain between bursts. | TBD | `time_to_sustained_vs_stock` / `latency_p95_vs_stock` verdicts in smoke_test (both currently FAIL — this gate prevents future "fixed!" claims without empirical proof) |
