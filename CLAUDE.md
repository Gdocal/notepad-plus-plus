...
| R16 (OPEN) | UI lags for ~19 s after window appears on 300+ tab sessions — p95 input latency 1.2 s, sustained-fast never reached within 25 s. Window is "technically responsive" (WM_NULL gets through between WM_TIMER ticks) but each user click/keystroke queues behind the current pump tick, so the user feels "not responding". Stock NPP is FULLY frozen for ~20 s then snaps to snappy; we're ~1 s faster to snappy but laggy throughout. Suspected cause: `TCM_INSERTITEM` at explicit `sessionIndex` in `processSessionInsertStep` is O(remaining tabs) per insert → O(N²) overall for the pump. Required for R1 (correct tab order) — fix needs a redesign (e.g. append-then-reorder once at end, or batch via TCM_SETITEM rotation). | TBD — current `_pendingSessionInserts` per-tick budget is 1, `addBufferAt` always inserts at `sessionIndex` | `time_to_sustained_vs_stock` / `latency_p95_vs_stock` verdicts in smoke_test (both currently FAIL) |
